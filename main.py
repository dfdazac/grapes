...
from modules.utils import slice_adjacency
from modules.utils import get_neighbors_data
from modules.utils import sample_neighborhoods_from_probs
...
def train(args: Arguments):
    ...
    tp = TraceProfiler(output=open(f'profiling/trace-{timestamp}.out', 'w'))

    with tqdm(range(args.max_epochs), disable=True) as loop:
        for epoch in loop:
            # with tp.traced():
            for batch_id in range(train_num_batches):
                ...
                for hop in range(args.num_hops):
                    ...
                    neighbors_data = get_neighbors_data(previous_nodes, adjacency)
                    neighborhoods, batch_nodes, neighbor_nodes = neighbors_data
                    ...
                    # We might want to defer this call to unique since all_nodes is used only later
